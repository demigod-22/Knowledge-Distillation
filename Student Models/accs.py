...
def prepare_data_PAMAP2(root_path='../../PAMAP2_Dataset/Protocol/subject10'):
    ...
    imu_locs = [5,6,7,22,23,24,39,40,41
            ] 
...
def train(net, epochs=10, batch_size=64, lr=0.01):
    ...
    for e in range(epochs):
        ...
        for batch in train_loader:
            ...
            loss = DistillationLoss(output, torch.argmax(targets,dim=1),net1(inputs))
            # print(output.shape)
            # print(targets.shape)
        ...
        with torch.no_grad():
            ...
            #     x, y = batch     
                ...
                val_loss = DistillationLoss(output, torch.argmax(targets,dim=1),net1(inputs))
